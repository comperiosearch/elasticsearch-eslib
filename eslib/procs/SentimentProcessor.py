...
def usage(err = None, rich= False):
    ...
    print >>OUT, "Usage:"
    print >>OUT, "  %s -h" % p
    print >>OUT, "  %s -s <sentimentFile> -f <fieldList> [-v] [-t <targetField>] [<fileNames>]" % p

    if rich:
        print >>OUT
        print >>OUT, "Field names are separated by commas. File names are normal command line arguments."
        print >>OUT, "If no file name is given then stdin is used instead. Field names can be suffixed"
        print >>OUT, "with ^weight where weight is a floating point number (0,1] to denote field weight."
        print >>OUT
        print >>OUT, "Sentiment description file has the format, by example:"
        print >>OUT, "{ \"strenghts\": {\"very\": 0.5, \"terribly\": 0.9},"
        print >>OUT, "  \"adjectives\": {\"good\": 0.5, \"bad\": -0.5} }"
