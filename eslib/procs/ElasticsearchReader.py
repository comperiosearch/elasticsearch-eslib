...
class ElasticsearchReader(eslib.DocumentProcessor):
    ...
    def write(self, doc):
        ...
        if self.outputFormat == "json":
            ...
            # Dump ID and fields
            fieldvalue = u" | ".join(doc["fields"].get(self.field) or [])
            if self.doctype:
...
def usage(err = None, rich= False):
    ...
    p = progname()
    print >>OUT, "Usage:"
    print >>OUT, "  %s -h                                                 More help" % p
    print >>OUT, "  %s -i <index> [-t <type>] [-l limit>] [-f <field>]    Show feed info" % p

    if rich:
        print >>OUT
        print >>OUT, "  Additional options:"
        print >>OUT, "    --since <ago>"
        print >>OUT, "    --before <ago>"
        print >>OUT, "    --timefield <field>"
        print >>OUT, "    --filter <filters>"
        print >>OUT, "    --format <format>"
        print >>OUT, "    --debug"
        print >>OUT
        print >>OUT, "  'ago' format is '1d', '2w', etc. Default 'timefield' to slice on is '_timestamp'."
        print >>OUT, "  Format for filter is, by example: 'category:politicians,party:democrats'."
        print >>OUT
        print >>OUT, "  Available formats for 'format':"
        print >>OUT, "    json (default), id, field (used by default if -f given)"
