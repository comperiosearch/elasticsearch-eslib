...
class PipelineStage(object):
    ...
    def write(self, text):
        # TODO: Ensure proper encoding for stdout
    ...
    def dout(self, text):
        if self.DEBUG: print >> sys.stderr, "%s: %s" % (self.name, text)

    def dout_raw(self, obj):
        if self.DEBUG: print >> sys.stderr, obj

    def vout(self, text):
        if self.VERBOSE: print >> sys.stderr, "%s: %s" % (self.name, text)

    def eout(self, text=None, exception=None):
        if not text and exception: text = exception.args[0]
        if not text: text = "???"
        print >> sys.stderr, "ERROR, %s: %s" % (self.name, text)
